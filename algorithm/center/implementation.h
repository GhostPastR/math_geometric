...
template<typename Figure, typename PointOut, typename Group, typename CoordinateSystem, std::size_t Dimension>
struct center{
...
template<typename Figure, typename PointOut>
struct center<Figure, PointOut, agl::group::group_polygon, agl::system_coordinat::cartesian, 2>{
    inline constexpr static auto get(const Figure &figure){
...
inline constexpr auto center(const Figure &figure){
    ...
    return dispatch::center<Figure, PointOut, group, type_cs, dimension>::get(figure);
}
