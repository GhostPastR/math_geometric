...
constexpr PointGeo common_survey_comp(Type range, TypeAngle omnibearing, const PointGeo &reference_point)
{
    ...
    auto _e2 = eccentricity2_1<Type>();
    auto _sinLat = ClassFunc::sin(reference_point.latitude());
    auto _os = sqrt(1.0 - _e2 * _sinLat * _sinLat);
    auto _sqrtE2 = sqrt(1.0 - _e2);
    auto _sinU1 = _sinLat * _sqrtE2 / _os;
    auto _cosU1 = ClassFunc::cos(reference_point.latitude()) / _os;
    auto _cosOmn = ClassFunc::cos(omnibearing);
    auto _sinOmn = ClassFunc::sin(omnibearing);

    auto _a0 = asin(_cosU1 * _sinOmn);
    auto _sin2q1 = 0.0;
    auto _cos2q1 = 1.0;
    ...
    auto _cos2a0 = ClassFunc::cos(_a0) * ClassFunc::cos(_a0);
    auto _k2 = eccentricity2_2<Type>() * _cos2a0;

    auto _k2_2 = _k2 * _k2;
    auto _kA = 1.0 + _k2 / 4.0 - 3.0 * _k2_2 / 64.0;
    auto _kBA = (_k2 / 4.0 - _k2_2 / 16.0) / _kA;
    auto _kC = _k2_2 / 128.0;
    ...
    auto _cos4q1_2q = 2 * _cos2q1_q * _cos2q1_q - 1;
    _q = _qD + _kBA * ClassFunc::sin(_q) * _cos2q1_q + (_kC / _kA) * ClassFunc::sin(2 * _q) * _cos4q1_2q;
    _cos2q1_q = _cos2q1 * ClassFunc::cos(_q) - _sin2q1 * ClassFunc::sin(_q);

    auto _cosQ = ClassFunc::cos(_q);
    auto _sinQ = ClassFunc::sin(_q) * _cosOmn;

    auto _cosU2 = _cosU1 * _cosQ - _sinU1 * _sinQ;
    auto _dY = ClassFunc::atan2(ClassFunc::sin(_q) * _sinOmn , _cosU2);
    auto _sinU2 = _sinU1 * _cosQ + _cosU1 * _sinQ;

    auto latitude = ClassFunc::atan(_sinU2 * ClassFunc::cos(_dY) / (_sqrtE2 * _cosU2));
...
constexpr auto geographic_inverse(const PointGeo &start, const PointGeo &stop)
    ...
    auto _e2 = eccentricity2_1<TypeAngle>();
    auto _sinLat = ClassFunc::sin(start.latitude());
    auto _os = sqrt( 1.0 - _e2 * _sinLat * _sinLat );
    auto _sinU1 = _sinLat * sqrt( 1.0 - _e2 ) / _os;
    auto _cosU1 = ClassFunc::cos(start.latitude()) / _os;
    _sinLat = ClassFunc::sin(stop.latitude());
    _os = sqrt( 1.0 - _e2 * _sinLat * _sinLat );
    auto _sinU2 = _sinLat * sqrt( 1.0 - _e2 ) / _os;
    auto _cosU2 =  ClassFunc::cos(stop.latitude()) / _os;

    auto _dL = (stop.longitude() - start.longitude());
    ...
    auto _g = acos( _n );
    auto _sinA0 = _cosU1 * ClassFunc::sin(_omnibearing);
    auto _a0 = ClassFunc::cos(ClassFunc::asin(_sinA0));
    _a0 *= _a0;
    auto _dY = _dL + _sinA0 * ( 0.5 + _e2 / 8.0 - ( _e2 / 16.0 ) * _a0 ) * _e2 * _g;
    ...
    auto _g1 = ClassFunc::atan2(_sinU1 , (_cosU1 * ClassFunc::cos(_omnibearing)));
    auto _e2h = eccentricity2_2<TypeAngle>();
    auto _k2 = _e2h * _a0;
    auto _k2_2 = _k2 * _k2;

    auto _range = ( 1.0 + _k2 / 4.0 - 3.0 * _k2_2 / 64.0 ) * semiminor_axis<TypeAngle> * _g -
