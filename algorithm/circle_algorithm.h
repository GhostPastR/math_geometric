...
// constexpr auto point_on_arc(const Type &cx, const Type &cy, const Type &radius, const AngleType &start, const AngleType &stop, Type distance)
//     -> std::optional<std::pair<Type, Type>>{
//     if(distance > length_arc(radius, start, stop)){
//         return std::nullopt;
//     }
//     const auto &[x,y] = point_algo::new_point(cx, cy, start, radius);
//     return point_algo::rotate(x, y, distance / radius, cx, cy);
// }
...
// }
// template<std::floating_point Type, std::floating_point AngleType>
// constexpr auto midplane(const Type &cx, const Type &cy, const Type &radius, const AngleType &start, const AngleType &stop) -> std::pair<Type, Type>{
//     return point_algo::new_point(cx, cy, magnitude_arc_angle(start, stop) / 2., radius);
// }
...
// }
// template<std::floating_point Type>
// constexpr bool point_appertain_circle(const Type &cx, const Type &cy, const Type &radius, const Type &x, const Type &y){
//     return algorithm::less_than_equal(algorithm::determine(x - cx, y - cy, -(y - cy), x - cx), std::pow(radius, 2));
// }
